...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS historico_dias (
            ...
            data TEXT NOT NULL,
            irrigou INTEGER NOT NULL,
            irrigacao_real_mm REAL NOT NULL,
            ...
            kl REAL NOT NULL,
            ks REAL NOT NULL,
            eto_mm REAL NOT NULL,
            etc_mm REAL NOT NULL,
            p_mm REAL NOT NULL,
            deplecao_mm REAL NOT NULL,
            taw_mm REAL NOT NULL,
            raw_mm REAL NOT NULL,
            lli_mm REAL NOT NULL,
            lbi_mm REAL NOT NULL,
            created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
            UNIQUE(plantio_id, data),
    ...
    cur.execute("""
        INSERT OR IGNORE INTO solos (nome, ucc, upmp, ds)
        VALUES
            ('Solo padrão', 0.32, 0.20, 1.25),
            ('Solo argiloso', 0.36, 0.22, 1.20),
            ('Solo franco', 0.30, 0.18, 1.30),
            ('Solo arenoso', 0.22, 0.10, 1.45)
    """)
...
def create_plantio(
    ...
    upmp: float,
    ds: float,
    z_override_m: Optional[float],
    f_override: Optional[float],
):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO plantios (
            nome, local, latitude, longitude, timezone, cultura_key,
            sistema_irrigacao, data_plantio, ucc, upmp, ds, z_override_m, f_override
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        nome, local, latitude, longitude, timezone, cultura_key,
        sistema_irrigacao, data_plantio.isoformat(), ucc, upmp, ds,
        z_override_m, f_override
    ))
    conn.commit()
    conn.close()
...
def get_plantio(plantio_id: int) -> Optional[dict]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM plantios WHERE id = ?", (plantio_id,))
    row = cur.fetchone()
    conn.close()
    if not row:
        return None
    ...
    cols = [
        "id", "nome", "local", "latitude", "longitude", "timezone", "cultura_key",
        "sistema_irrigacao", "data_plantio", "ucc", "upmp", "ds",
        "z_override_m", "f_override", "created_at"
    ]
    return dict(zip(cols, row))


def get_irrigation_map(plantio_id: int) -> Dict[date, float]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        SELECT data, irrigacao_real_mm
        FROM historico_dias
        WHERE plantio_id = ?
    """, (plantio_id,))
    rows = cur.fetchall()
    ...
    for data_txt, mm in rows:
        resultado[to_date(data_txt)] = float(mm or 0.0)
    return resultado


def get_last_saved_day(plantio_id: int) -> Optional[dict]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        SELECT data, dap, deplecao_mm, irrigou, irrigacao_real_mm, eto_mm, etc_mm, p_mm
        FROM historico_dias
        WHERE plantio_id = ?
        ORDER BY data DESC
        LIMIT 1
    ...
    cols = ["data", "dap", "deplecao_mm", "irrigou", "irrigacao_real_mm", "eto_mm", "etc_mm", "p_mm"]
    return dict(zip(cols, row))


def upsert_day_result(plantio_id: int, irrigou: bool, result: ResultDay):
    conn = get_conn()
    cur = conn.cursor()

    cur.execute("""
        INSERT INTO historico_dias (
            plantio_id, data, irrigou, irrigacao_real_mm, dap, fase, kc, akc, kl, ks,
            eto_mm, etc_mm, p_mm, deplecao_mm, taw_mm, raw_mm, lli_mm, lbi_mm
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(plantio_id, data) DO UPDATE SET
            irrigou = excluded.irrigou,
            irrigacao_real_mm = excluded.irrigacao_real_mm,
            dap = excluded.dap,
            fase = excluded.fase,
            kc = excluded.kc,
            akc = excluded.akc,
            ...
            p_mm = excluded.p_mm,
            deplecao_mm = excluded.deplecao_mm,
            taw_mm = excluded.taw_mm,
            raw_mm = excluded.raw_mm,
            lli_mm = excluded.lli_mm,
            lbi_mm = excluded.lbi_mm
    """, (
        plantio_id,
        result.data.isoformat(),
        1 if irrigou else 0,
        result.irrigacao_real_mm,
        result.dap,
        result.fase,
        result.kc,
        result.akc,
...
def load_history_df(plantio_id: int) -> pd.DataFrame:
    conn = get_conn()
    df = pd.read_sql_query("""
        SELECT
            data AS "Data",
            ...
            p_mm AS "P (mm)",
            irrigou AS "Irrigou",
            irrigacao_real_mm AS "I_real (mm)",
            deplecao_mm AS "Dr (mm)",
            taw_mm AS "TAW (mm)",
            raw_mm AS "RAW (mm)",
            lli_mm AS "LLI (mm)",
            lbi_mm AS "LBI (mm)"
        ...
        WHERE plantio_id = ?
        ORDER BY data
    """, conn, params=(plantio_id,))
    conn.close()

    if not df.empty:
        df["Irrigou"] = df["Irrigou"].map({1: "Sim", 0: "Não"})
...
def create_solo(nome: str, ucc: float, upmp: float, ds: float):
    ...
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO solos (nome, ucc, upmp, ds)
        VALUES (?, ?, ?, ?)
    """, (nome, ucc, upmp, ds))
    conn.commit()
    conn.close()
...
def delete_solo(solo_id: int):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("DELETE FROM solos WHERE id = ?", (solo_id,))
    conn.commit()
    conn.close()


def delete_history_day(plantio_id: int, data_iso: str):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        DELETE FROM historico_dias
        WHERE plantio_id = ? AND data = ?
    """, (plantio_id, data_iso))
    conn.commit()
    conn.close()


def delete_all_history(plantio_id: int):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        DELETE FROM historico_dias
...
def delete_plantio(plantio_id: int):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("DELETE FROM historico_dias WHERE plantio_id = ?", (plantio_id,))
    cur.execute("DELETE FROM plantios WHERE id = ?", (plantio_id,))
    conn.commit()
    conn.close()
...
with aba1:
    ...
    with st.form("form_novo_plantio"):
        ...
        usar_f = c13.checkbox("Usar f manual")

        z_override_m = st.number_input("Z manual (m)", min_value=0.0, value=0.35, format="%.3f") if usar_z else None
        f_override = st.number_input("f manual", min_value=0.0, max_value=1.0, value=0.50, format="%.3f") if usar_f else None
...
with aba2:
    ...
    if plantios_df.empty:
        ...
        opcoes = {
            f"ID {row['id']} - {row['nome']} - {row['cultura_key']} - plantio {row['data_plantio']}": int(row["id"])
...
with aba3:
    ...
    if plantios_df.empty:
        st.info("Ainda não há plantios cadastrados.")
    else:
        opcoes_hist = {
            f"ID {row['id']} - {row['nome']} - {row['cultura_key']} - plantio {row['data_plantio']}": int(row["id"])
            for _, row in plantios_df.iterrows()
...
with aba4:
    ...
    with subaba2:
        ...
        if not solos_df.empty:
            ...
            opcoes_delete_solo = {
                f"ID {row['id']} - {row['nome']}": int(row["id"])
                for _, row in solos_df.iterrows()
            }
